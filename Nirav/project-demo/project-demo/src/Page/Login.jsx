...
const Login = () => {
    ...
    const [userDetail, setUserDetail] = useState(localStorage.getItem('UserDetail') ? JSON.parse(localStorage.getItem('UserDetail')) : [])

    const [api, contextHolder] = notification.useNotification();
    const openNotification = (placement) => {
        api.info({
            description: 'Envalid Email and Password',
            placement,
        });
    };
    ...
    const onFinish = (data) => {
        console.log(data)
        let temp = userDetail.find((e) => e.email === data.email && e.password === data.password)
        if (temp) {
            localStorage.setItem('token','eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoiSm9obiBEb2UiLCJpYXQiOjE1MTYyMzkwMjIsImV4cCI6MTUxNjIzOTAyMn0.yOZC0rjfSopcpJ-d3BWE8-BkoLR_SCqPdJpq8Wn-1Mc')
            navigate('/home')
        } else {
            ...
            <div class="container" id="container">
                ...
                {contextHolder}
                    <Form name="basic" form={form}
                        ...
                        autoComplete="off"
                    >
                        <p style={{fontFamily:'initial', fontSize:'38px'}}><big>S</big>ign In</p>
                ...
                <div class="overlay-container">
                    <div class="overlay">
                        <div class="overlay-panel overlay-right">
                            ...
                            <button class="ghost" id="signUp" onClick={() => navigate('/signUp')}>Sign Up</button>
                        </div>
                    </div>
                </div>
            </div>
